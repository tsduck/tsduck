...
class CodecsTest: public tsunit::Test
{
    ...
    TSUNIT_DECLARE_TEST(HEVCConformanceWindow);
};
...
TSUNIT_DEFINE_TEST(HEVCConformanceWindow)
{
    // Synthetic 1920x1080 HEVC sequence generated by FFmpeg VideoToolbox. The
    // coded height is 1088 and the bottom conformance-window offset removes
    // eight luma lines.
    static const uint8_t data[] = {
        0x42, 0x01, 0x01, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0xB0, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03,
        0x00, 0x78, 0xA0, 0x03, 0xC0, 0x80, 0x11, 0x07, 0xCB, 0x88, 0x17, 0xB9, 0x16, 0x45, 0x2F, 0xFC,
        0xB9, 0xFC, 0x4F, 0xEB, 0x01, 0x6A, 0x04, 0x04, 0x04, 0x01,
    };

    const ts::HEVCSequenceParameterSet sps(data, sizeof(data));

    TSUNIT_ASSERT(sps.valid);
    TSUNIT_EQUAL(1920, sps.pic_width_in_luma_samples);
    TSUNIT_EQUAL(1088, sps.pic_height_in_luma_samples);
    TSUNIT_EQUAL(1, sps.conformance_window_flag);
    TSUNIT_EQUAL(0, sps.conf_win_left_offset);
    TSUNIT_EQUAL(0, sps.conf_win_right_offset);
    TSUNIT_EQUAL(0, sps.conf_win_top_offset);
    TSUNIT_EQUAL(4, sps.conf_win_bottom_offset);
    TSUNIT_EQUAL(2, sps.subWidthC());
    TSUNIT_EQUAL(2, sps.subHeightC());
    TSUNIT_EQUAL(1920, sps.frameWidth());
    TSUNIT_EQUAL(1080, sps.frameHeight());
}
