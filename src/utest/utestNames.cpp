...
TSUNIT_DEFINE_TEST(DID)
{
    ...
    TSUNIT_EQUAL(u"unknown (0x83)", ts::DIDName(ts::DID_EACEM_LCN));
    TSUNIT_EQUAL(u"Logical Channel Number", ts::DIDName(ts::DID_EACEM_LCN, ts::NamesFlags::NAME,
                                                        ts::DescriptorContext(ts::TID_NULL, ts::Standards::DVB, ts::CASID_NULL, ts::REGID_NULL, ts::PDS_EACEM)));
    TSUNIT_EQUAL(u"0x83 (Logical Channel Number)", ts::DIDName(ts::DID_EACEM_LCN, ts::NamesFlags::FIRST,
                                                               ts::DescriptorContext(ts::TID_NULL, ts::Standards::DVB, ts::CASID_NULL, ts::REGID_NULL, ts::PDS_EACEM)));
}
...
TSUNIT_DEFINE_TEST(StreamType)
{
    ...
    TSUNIT_EQUAL(u"DTS-HD Master Audio", ts::StreamTypeName(ts::ST_SCTE35_SPLICE, ts::NamesFlags::NAME, ts::REGID_HDMV));

    ts::DuckContext duck;
    ts::DescriptorList dlist(nullptr);
    dlist.add(duck, ts::RegistrationDescriptor(ts::REGID_CUEI));
    TSUNIT_EQUAL(u"SCTE 35 Splice Info", ts::StreamTypeName(ts::ST_SCTE35_SPLICE, ts::NamesFlags::NAME, dlist));

    dlist.add(duck, ts::RegistrationDescriptor(ts::REGID_HDMV));
    TSUNIT_EQUAL(u"DTS-HD Master Audio", ts::StreamTypeName(ts::ST_SCTE35_SPLICE, ts::NamesFlags::NAME, dlist));
}
