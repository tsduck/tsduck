...
void ts::T2MIPlugin::handleT2MINewPID(T2MIDemux& demux, const PMT& pmt, PID pid, const T2MIDescriptor& desc)
{
    ...
    if (!_extract_pid.has_value()) {
        if (_extract || _log) {
            tsp->verbose(u"using PID %n to extract T2-MI stream", pid);
        }
        _extract_pid = pid;
        _demux.addPID(pid);
    }
    ...
    if (_identify) {
        tsp->info(u"found T2-MI PID %n", pid);
...
void ts::T2MIPlugin::handleT2MIPacket(T2MIDemux& demux, const T2MIPacket& pkt)
{
    ...
    if (_log && _extract_pid == pid) {
        ...
        tsp->info(u"PID %n, packet type: %s, size: %d bytes, packet count: %d, superframe index: %d, frame index: %d%s",
    ...
    if (_extract && _extract_pid == pid && hasPLP) {
        if (!_extract_plp.has_value()) {
            ...
            _extract_plp = plp;
            tsp->verbose(u"extracting PLP %n", plp);
        }
        if (_extract_plp == plp) {
    ...
    if (_identify && hasPLP) {
        ...
        if (!plps.test(plp)) {
            plps.set(plp);
            tsp->info(u"PID %n, found PLP %d", pid, plp);
        }
    }
