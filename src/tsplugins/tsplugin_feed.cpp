...
void ts::FeedPlugin::handleTable(SectionDemux& demux, const BinaryTable& table)
{
            ...
            if (pmt.isValid()) {
                ...
                for (const auto& it : pmt.streams) {
                    if (it.second.stream_type == _stream_type) {
                        tsp->debug(u"possible tunnel PID %n in service %n", it.first, pmt.service_id);
                ...
                if (pmt.descs.search(duck, DID_SERVICE, sd) < pmt.descs.size()) {
                    tsp->debug(u"service %n has type %n", pmt.service_id, sd.service_type);
        ...
        case TID_SDT_ACT: {
            ...
            if (sdt.isValid()) {
                ...
                for (const auto& it : sdt.services) {
                    ...
                    if (type != 0) {
                        tsp->debug(u"service %n has type %n", it.first, type);
    ...
    if (_extract_pid == PID_NULL) {
        ...
        for (const auto& itype : _service_types) {
            if (itype.second == _service_type) {
                ...
                if (ipid != _service_pids.end() && ipid->second != PID_NULL) {
                    ...
                    tsp->verbose(u"extracting feed from PID %n, service id %n", _extract_pid, itype.first);
                    return;
                }
            }
        }
        ...
        if (!_all_services.empty()) {
            ...
            if (got_them_all) {
                tsp->error(u"no service found with type %n with a PID with stream type %n", _service_type, _stream_type);
                _abort = true;
            }
        }
    }
