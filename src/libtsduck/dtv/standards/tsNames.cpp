...
ts::UString ts::names::TID(const DuckContext& duck, uint8_t tid, uint16_t cas, NamesFlags flags)
{
    ...
    if (repo->nameExists(section, tidValue | casValue)) {
        ...
        return repo->nameFromSection(section, tidValue | casValue, flags);
    }
    else if (repo->nameExists(section, tidValue)) {
        // Found without standard, without CAS. Keep this value.
        return repo->nameFromSection(section, tidValue, flags);
    }
    else {
        ...
        for (Standards mask = Standards(1); mask != Standards::NONE; mask <<= 1) {
            ...
            if (!foundWithSupportedStandard || supportedStandard) {
                ...
                if (repo->nameExists(section, tidValue | stdValue | casValue)) {
                    ...
                    name = repo->nameFromSection(section, tidValue | stdValue | casValue, flags);
                }
                else if (repo->nameExists(section, tidValue | stdValue)) {
                    // Found with that standard, without CAS.
                    name = repo->nameFromSection(section, tidValue | stdValue, flags);
                }
                if (!name.empty()) {
                    // A name was found.
                    if (!foundWithSupportedStandard && supportedStandard) {
...
ts::UString ts::names::DID(uint8_t did, uint32_t pds, uint8_t tid, NamesFlags flags)
{
    if (did >= 0x80 && pds != 0 && pds != PDS_NULL) {
        ...
        return NameFromDTV(u"DescriptorId", (NamesFile::Value(pds) << 8) | NamesFile::Value(did), flags);
    }
    else if (tid != 0xFF) {
        ...
        return NameFromDTVWithFallback(u"DescriptorId", fullValue, NamesFile::Value(did), flags);
    }
    else {
        return NameFromDTV(u"DescriptorId", NamesFile::Value(did), flags);
    }
}
...
{
    return NameFromDTV(u"DVBExtendedDescriptorId", NamesFile::Value(edid), flags);
}
...
ts::UString ts::names::StreamType(uint8_t type, NamesFlags flags, uint32_t regid)
{
    ...
    return repo->nameFromSection(u"StreamType", fullValue, flags);
}
...
ts::UString ts::names::CASId(const DuckContext& duck, uint16_t id, NamesFlags flags)
{
    ...
    if (bool(duck.standards() & Standards::ISDB)) {
        const UString name(NameFromDTV(u"ARIBCASystemId", NamesFile::Value(id), flags | NamesFlags::NO_UNKNOWN));
        if (!name.empty()) {
            return name;
        }
    }
    // Not ISDB or not found in ISDB, use standard CAS names.
    return NameFromDTV(u"CASystemId", NamesFile::Value(id), flags);
}
