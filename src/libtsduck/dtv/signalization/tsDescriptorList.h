...
namespace ts {
    ...
    public:
        ...
        //! The returned list can contain duplicates if the duplicates are present in the
        //! descriptor list.
        //!
        void getAllRegistrations(std::vector<REGID>& regids) const;
        ...
        //!
        //! A class implementing DescriptorContext to search for private descriptors registration ids.
        //! No public constructor is available. Use DescriptorList::lookupContext().
        //!
        class TSDUCKDLL LookupContext: public DescriptorContext
        {
            TS_DEFAULT_COPY_MOVE(LookupContext);
        public:
            // Implementation of DescriptorContext.
            virtual bool getPrivateIds(REGID* regid, PDS* pds) const override;

        private:
            const DescriptorList* _dlist = nullptr;
            size_t _start_index = 0;

            // Private constructor, used by lookupContext().
            LookupContext(const DuckContext& duck, const DescriptorList* dlist, size_t start_index, CASID cas);
            friend class DescriptorList;
        };

        //!
        //! Return a lookup object to search private descriptors registration ids in this descriptor list.
        //! @param [in] duck TSDuck execution context.
        //! @param [in] index Start index in the descriptor list. MPEG registration descriptors
        //! and DVB private data specifier descriptors are searched backward, starting at the
        //! specified index in the list, or at the last element in the descriptor list if @a index
        //! is larger than the list size. MPEG registration descriptors are also searched in the
        //! associated top-level descriptor list in the table, if one is known.
        //! @param [in] cas Optional CAS id in the context of the descriptor list.
        //! @return A DescriptorContext object. This object is valid as long at this DescriptorList
        //! object is valid.
        //!
        LookupContext lookupContext(const DuckContext& duck, size_t index, CASID cas = CASID_NULL) const { return LookupContext(duck, this, index, cas); }
