...
bool ts::DescriptorList::merge(DuckContext& duck, const AbstractDescriptor& desc)
{
    ...
    const EDID edid = desc.edid(_table);
    ...
    if (&other != this) {
        ...
        for (size_t index = 0; index < other._list.size(); ++index) {
            ...
            if (bindesc != nullptr && bindesc->isValid()) {
                ...
                const AbstractDescriptorPtr dp(bindesc->deserialize(duck, other.lookupContext(duck, index)));
                if (dp == nullptr || dp->duplicationMode() == DescriptorDuplication::ADD_ALWAYS) {
                    // Cannot be deserialized or simply add the descriptor.
                    addPrivateIdentifier(dp->edid(other._table));
                    add(bindesc);
                }
                else {
...
void ts::DescriptorList::getAllRegistrations(std::vector<REGID>& regids) const
{
    regids.clear();
    for (const auto& dp : _list) {
...
//----------------------------------------------------------------------------
// A class implementing DescriptorContext to search for private descriptors
//----------------------------------------------------------------------------

// Constructor. Used only in lookupContext(), dlist is never null.
ts::DescriptorList::LookupContext::LookupContext(const DuckContext& duck, const DescriptorList* dlist, size_t start_index, CASID cas) :
    DescriptorContext(dlist->tableId(), duck.standards() | dlist->tableStandards(), duck.casId(cas)),
    _dlist(dlist),
    _start_index(start_index)
{
}

bool ts::DescriptorList::LookupContext::getPrivateIds(REGID* regid, PDS* pds) const
{
    // Reset output variables.
    if (regid == nullptr && pds == nullptr) {
    ...
    bool regid_first = false;
    bool regid_done = regid == nullptr;
    bool pds_done = pds == nullptr;
    const DescriptorList* dlist = _dlist;

    // Loop on current descriptor list and top-level descriptor list for REGID.
    while (dlist != nullptr && (!regid_done || !pds_done)) {
        size_t index = std::min(_start_index + 1, dlist->size());
        while (index-- > 0 && (!regid_done || !pds_done)) {
            const DescriptorPtr& desc((*dlist)[index]);
            if (desc != nullptr && desc->isValid() && desc->payloadSize() >= 4) {
                const DID did = desc->tag();
                if (!regid_done && did == DID_MPEG_REGISTRATION) {
                    *regid = GetUInt32(desc->payload());
                    regid_done = true;
                    regid_first = pds != nullptr && *pds == PDS_NULL;
                }
                else if (!pds_done && did == DID_DVB_PRIV_DATA_SPECIF) {
                    *pds = GetUInt32(desc->payload());
                    pds_done = true;
                }
            }
        }
        // End of descriptor list. Get top-level list if it exists and is different.
        const AbstractTable* table = dlist->table();
        const DescriptorList* next = table == nullptr ? nullptr : table->topLevelDescriptorList();
        dlist = next == dlist ? nullptr : next;
        // Don't search PDS in top-level descriptor list, only REGID are inherited.
        pds_done = true;
    }

    // Return true only if two values were found and REGID comes before PDS.
    return regid_first && regid != nullptr && *regid != REGID_NULL && pds != nullptr && *pds != REGID_NULL;
}
...
bool ts::DescriptorList::toXML(DuckContext& duck, xml::Element* parent) const
{
    ...
    for (size_t index = 0; index < _list.size(); ++index) {
        if (_list[index] == nullptr || _list[index]->toXML(duck, parent, lookupContext(duck, index), false) == nullptr) {
            success = false;
        }
    }
    return success;
