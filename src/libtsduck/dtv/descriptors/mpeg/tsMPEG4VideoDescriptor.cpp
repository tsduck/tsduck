...
#define MY_EDID     ts::EDID::Regular(ts::DID_MPEG_MPEG4_VIDEO, ts::Standards::MPEG)
...
void ts::MPEG4VideoDescriptor::DisplayDescriptor(TablesDisplay& disp, const ts::Descriptor& desc, PSIBuffer& buf, const UString& margin, const ts::DescriptorContext& context)
{
    if (buf.canReadBytes(1)) {
        disp << margin << UString::Format(u"MPEG-4 Video profile and level: %n", buf.getUInt8()) << std::endl;
    }
}
