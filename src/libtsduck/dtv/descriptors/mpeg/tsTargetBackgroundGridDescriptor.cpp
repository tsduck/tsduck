...
#define MY_EDID     ts::EDID::Regular(ts::DID_MPEG_TGT_BG_GRID, ts::Standards::MPEG)
...
void ts::TargetBackgroundGridDescriptor::DisplayDescriptor(TablesDisplay& disp, const ts::Descriptor& desc, PSIBuffer& buf, const UString& margin, const ts::DescriptorContext& context)
{
    if (buf.canReadBytes(4)) {
        disp << margin << UString::Format(u"Size: %d", buf.getBits<uint16_t>(14));
        disp << UString::Format(u"x%d", buf.getBits<uint16_t>(14));
        disp << ", aspect ratio: " << NameFromSection(u"dtv", u"mpeg2.aspect_ratio", buf.getBits<uint8_t>(4), NamesFlags::DEC_VALUE_NAME) << std::endl;
    }
}
