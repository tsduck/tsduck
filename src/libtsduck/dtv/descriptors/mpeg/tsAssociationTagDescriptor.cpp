...
#define MY_EDID     ts::EDID::Regular(ts::DID_MPEG_ASSOCIATION_TAG, ts::Standards::MPEG)
...
void ts::AssociationTagDescriptor::DisplayDescriptor(TablesDisplay& disp, const ts::Descriptor& desc, PSIBuffer& buf, const UString& margin, const ts::DescriptorContext& context)
{
    if (buf.canReadBytes(5)) {
        disp << margin << UString::Format(u"Association tag: %n", buf.getUInt16());
        disp << UString::Format(u", use: %n", buf.getUInt16()) << std::endl;
