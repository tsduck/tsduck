...
#define MY_EDID     ts::EDID::Regular(ts::DID_MPEG_METADATA_POINTER, ts::Standards::MPEG)
...
void ts::MetadataPointerDescriptor::DisplayDescriptor(TablesDisplay& disp, const ts::Descriptor& desc, PSIBuffer& buf, const UString& margin, const ts::DescriptorContext& context)
{
    ...
    if (!buf.canReadBytes(2)) {
        ...
        if (flags == 1 && buf.remainingReadBytes() >= 4) {
            disp << margin << UString::Format(u"Transport stream location: %n", buf.getUInt16()) << std::endl;
