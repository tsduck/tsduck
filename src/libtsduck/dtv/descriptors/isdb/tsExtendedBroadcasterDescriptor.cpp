...
#define MY_EDID     ts::EDID::Regular(ts::DID_ISDB_EXT_BROADCAST, ts::Standards::ISDB)
...
void ts::ExtendedBroadcasterDescriptor::DisplayDescriptor(TablesDisplay& disp, const ts::Descriptor& desc, PSIBuffer& buf, const UString& margin, const ts::DescriptorContext& context)
{
    if (buf.canReadBytes(1)) {
        ...
        if ((btype == 0x01 || btype == 0x02) && buf.canReadBytes(3)) {
            ...
            while (aff_count-- > 0 && buf.canReadBytes(1)) {
                disp << margin << UString::Format(u"- %s id: %n", btype == 0x02 ? u"Sound broadcast affiliation" : u"Affiliation", buf.getUInt8()) << std::endl;
            }
