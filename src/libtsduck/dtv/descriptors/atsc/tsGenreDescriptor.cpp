...
#define MY_EDID     ts::EDID::Regular(ts::DID_ATSC_GENRE, ts::Standards::ATSC)
...
void ts::GenreDescriptor::DisplayDescriptor(TablesDisplay& disp, const ts::Descriptor& desc, PSIBuffer& buf, const UString& margin, const ts::DescriptorContext& context)
{
    if (buf.canReadBytes(1)) {
        ...
        while (count-- > 0 && buf.canReadBytes(1)) {
            disp << margin << " - Attribute: " << DataName(MY_XML_NAME, u"code", buf.getUInt8(), NamesFlags::VALUE_NAME) << std::endl;
        }
    }
}
