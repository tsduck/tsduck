...
#define MY_EDID     ts::EDID::Regular(ts::DID_ATSC_TIME_SHIFT, ts::Standards::ATSC)
...
void ts::ATSCTimeShiftedServiceDescriptor::DisplayDescriptor(TablesDisplay& disp, const ts::Descriptor& desc, PSIBuffer& buf, const UString& margin, const ts::DescriptorContext& context)
{
    if (buf.canRead()) {
        buf.skipBits(3);
        const size_t count = buf.getBits<size_t>(5);
...
bool ts::ATSCTimeShiftedServiceDescriptor::analyzeXML(DuckContext& duck, const xml::Element* element)
{
    ...
    for (auto& xserv : element->children(u"service", &ok, 0, MAX_ENTRIES)) {
        ...
        ok = xserv.getIntAttribute(entry.time_shift, u"time_shift", true, 0, 0, 0x03FF) &&
             xserv.getIntAttribute(entry.major_channel_number, u"major_channel_number", true, 0, 0, 0x03FF) &&
             xserv.getIntAttribute(entry.minor_channel_number, u"minor_channel_number", true, 0, 0, 0x03FF);
    }
    return ok;
}
