...
#define MY_EDID     ts::EDID::TableSpecific(ts::DID_AIT_TRANSPORT_PROTO, ts::Standards::DVB, ts::TID_AIT)
...
void ts::TransportProtocolDescriptor::DisplayDescriptor(TablesDisplay& disp, const ts::Descriptor& desc, PSIBuffer& buf, const UString& margin, const ts::DescriptorContext& context)
{
    if (buf.canReadBytes(3)) {
        const uint16_t proto = buf.getUInt16();
        disp << margin << "Protocol id: " << NameFromSection(u"dtv", u"MHPTransportProtocolId", proto, NamesFlags::HEX_DEC_VALUE_NAME) << std::endl;
