...
ts::BitRateRegulator::~BitRateRegulator()
{
}
...
void ts::BitRateRegulator::start()
{
    ...
    report().log(_log_level, u"minimum packet burst duration is %s", _burst_min);
...
void ts::BitRateRegulator::handleNewBitrate()
{
    ...
    report().debug(u"new regulation, burst: %'s, %'d packets, measurement period: %'s", _burst_duration, burst_pkt_max, _period_duration);
}
...
void ts::BitRateRegulator::regulate(const BitRate& current_bitrate, bool& flush, bool& bitrate_changed)
{
    ...
    if (_cur_bitrate != old_bitrate || _starting) {
        if (_cur_bitrate == 0) {
            report().log(_log_level, u"unknown bitrate, cannot regulate.");
        }
        else {
            report().log(_log_level, u"regulated at bitrate %'d b/s", _cur_bitrate.toInt());
        }
    }
    _starting = false;
