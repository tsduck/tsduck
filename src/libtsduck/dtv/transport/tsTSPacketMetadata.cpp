...
bool ts::TSPacketMetadata::hasISDBT(DuckContext& duck) const
{
    return _aux_data_size >= 8 && bool(duck.standards() & Standards::ISDB);
}
...
void ts::TSPacketMetadata::displayISDBT(DuckContext& duck, std::ostream& strm, const UString& margin, bool check_standards) const
{
    if (_aux_data_size >= 8 && (!check_standards || bool(duck.standards() & Standards::ISDB))) {
        PSIBuffer buf(duck, _aux_data, _aux_data_size);
