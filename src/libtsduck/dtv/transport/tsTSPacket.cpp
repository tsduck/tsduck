...
void ts::TSPacket::Copy(TSPacket* dest, const TSPacket* source, size_t count)
{
    assert(dest != nullptr);
    assert(source != nullptr);
    MemCopy(dest->b, source->b, count * PKT_SIZE);
}
...
bool ts::TSPacket::setPayloadSize(size_t size, bool shift_payload, uint8_t pad)
{
    ...
    if (size == plSize) {
        ...
        if (shift_payload) {
            ...
            MemCopy(b + PKT_SIZE - size, b + PKT_SIZE - plSize, size);
        }
        if ((b[3] & 0x20) == 0) {
        ...
        if (shift_payload) {
            ...
            MemCopy(b + PKT_SIZE - size, b + PKT_SIZE - plSize, plSize);
...
bool ts::TSPacket::setPrivateData(const void* data, size_t size, bool shift_payload)
{
    ...
    if (!hasData) {
        ...
        endAF = 5 + b[4];
        MemCopy(b + endNewData, b + offset, endAF - endNewData);
    }
    else {
        ...
        if (endNewData < endPreviousData) {
            ...
            MemCopy(b + endNewData, b + endPreviousData, endAF - endPreviousData);
            ...
            endAF = 5 + b[4];
            MemCopy(b + endNewData, b + endPreviousData, endAF - endNewData);
        }
    }
...
void ts::TSPacket::deleteFieldFromAF(size_t offset, size_t size, uint32_t flag)
{
    if (offset > 0) {
        ...
        b[5] &= ~flag;
        // Shift the adaptation field down. With MemCopy(), the memory regions may overlap.
        MemCopy(b + offset, b + offset + size, 4 + afSize - offset - size);
...
bool ts::TSPacket::setSpliceCountdown(int8_t count, bool shift_payload)
{
    ...
    if (offset == 0) {
        ...
        MemCopy(b + offset + 1, b + offset, 4 + getAFSize() - offset - 1);
    }
...
bool ts::TSPacket::setPCR(const uint64_t &pcr, bool shift_payload)
{
    ...
    if (offset == 0) {
        ...
        MemCopy(b + offset + PCR_BYTES, b + offset, 4 + getAFSize() - offset - PCR_BYTES);
    }
...
bool ts::TSPacket::samePayload(const TSPacket& other) const
{
    if (!hasPayload() || !other.hasPayload()) {
        ...
        const size_t plsize = getPayloadSize();
        return other.getPayloadSize() == plsize && MemEqual(getPayload(), other.getPayload(), plsize);
    }
}
...
bool ts::TSPacket::isDuplicate(const TSPacket& other) const
{
    ...
    return hasPayload() &&
        getPID() != PID_NULL &&
        MemEqual(b, other.b, 6) &&
        MemEqual(b + offset, other.b + offset, PKT_SIZE - offset);
}
...
std::ostream& ts::TSPacket::display(std::ostream& strm, uint32_t flags, size_t indent, size_t max_size) const
{
        ...
        if (adaptation_field_extension_flag && buf.canReadBits(8)) {
            ...
            if (!af_descriptor_not_present_flag) {
                ...
                while (buf.canReadBytes(2)) {
                    strm << margin << "- Tag: " << NameFromDTV(u"ts.af_descriptor_tag", buf.getUInt8(), NamesFlags::VALUE_NAME) << std::endl;
    ...
    if (startPES() && (flags & DUMP_PES_HEADER)) {
        ...
        strm << margin << "---- PES Header ----" << std::endl
             << margin << "Stream id: " << NameFromDTV(u"pes.stream_id", sid, NamesFlags::VALUE_NAME) << std::endl
