...
bool ts::TeletextPlugin::start()
{
    ...
    getIntValue(_max_frames, u"max-frames", 0);
    getValue(_language, u"language");
    getPathValue(_out_file, u"output-file");
    _demux.setAddColors(present(u"colors"));

    // Create the output file.
    if (_out_file.empty()) {
        // No output file specified, use standard output.
        _srt_output.setStream(&std::cout);
    }
    else if (!_srt_output.open(_out_file, *this)) {
...
void ts::TeletextPlugin::handleTeletextMessage(TeletextDemux& demux, const TeletextFrame& frame)
{
    ...
    if (frame.page() == _page) {
        // Format frame as SRT.
        _srt_output.addFrame(frame.showTimestamp(), frame.hideTimestamp(), frame.lines());

        // Count frames and stop when the maximum is reached.
        if (_max_frames > 0 && frame.frameCount() >= _max_frames) {
            _abort = true;
        }
    }
}
...
ts::PacketProcessStatus ts::TeletextPlugin::processPacket(TSPacket& pkt, TSPacketMetadata& pkt_data)
{
