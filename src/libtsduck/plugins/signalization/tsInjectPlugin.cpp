    ...
    ProcessorPlugin(tsp_, u"Inject tables and sections in a TS", u"[options] input-file[=rate] ...")
{
    ...
    help(u"evaluate-interval",
         u"When used with --replace and when specific repetition rates are specified for some input files, "
         u"the bitrate of the target PID is re-evaluated on a regular basis. "
         u"The value of this option specifies the number of packet in the target PID before re-evaluating its bitrate. "
         u"The default is " + UString::Decimal(DEFAULT_EVALUATE_INTERVAL) + u" packets.");

    option(u"force-crc", 'f');
    help(u"force-crc",
         u"Force recomputation of CRC32 in long sections. "
         u"Ignore CRC32 values in input file.");
...
bool ts::InjectPlugin::getOptions()
{
    ...
    getIntValue(_eval_interval, u"evaluate-interval", DEFAULT_EVALUATE_INTERVAL);
...
ts::PacketProcessStatus ts::InjectPlugin::processPacket(TSPacket& pkt, TSPacketMetadata& pkt_data)
{
