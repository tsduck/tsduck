    ...
    ProcessorPlugin(tsp_, u"Permanently recompute bitrate based on PCR analysis", u"[options]")
{
    ...
    help(u"min-pcr",
         ...
         u"minimum number of PID (default: " + UString::Decimal(DEFAULT_MIN_PCR_COUNT) + u").");

    option(u"min-pid", 0, POSITIVE);
    help(u"min-pid",
         u"Minimum number of PID to get PCR from (default: " + UString::Decimal(DEFAULT_MIN_PID_COUNT) + u").");
}
...
bool ts::PCRBitratePlugin::start()
{
    ...
    const size_t min_pcr = intValue<size_t>(u"min-pcr", DEFAULT_MIN_PCR_COUNT);
    const size_t min_pid = intValue<size_t>(u"min-pid", DEFAULT_MIN_PID_COUNT);
    if (present(u"dts")) {
