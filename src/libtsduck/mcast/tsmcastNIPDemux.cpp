...
ts::mcast::NIPDemux::NIPDemux(DuckContext& duck, NIPHandlerInterface* handler) :
    _duck(duck),
    _handler(handler)
{
}
...
void ts::mcast::NIPDemux::addProtocolSession(const TransportProtocol& protocol, const FluteSessionId& session)
{
...
bool ts::mcast::NIPDemux::isFiltered(const IPAddress& source, const IPSocketAddress& destination) const
{
...
void ts::mcast::NIPDemux::handleFluteFile(const FluteFile& file)
{
    ...
    // Finally forward to the NIP handler.
    if (_handler != nullptr) {
        _handler->handleFluteFile(file);
    }
}


//----------------------------------------------------------------------------
// Other FLUTE handlers only forward to the NIP handler.
//----------------------------------------------------------------------------

void ts::mcast::NIPDemux::handleFluteFDT(const FluteFDT& fdt)
{
    if (_handler != nullptr) {
...
void ts::mcast::NIPDemux::handleFluteNACI(const NIPActualCarrierInformation& naci)
{
    if (_handler != nullptr) {
...
void ts::mcast::NIPDemux::processGatewayConfiguration(const GatewayConfiguration& mgc)
{
    ...
    // Finally forward to the NIP handler.
    if (_handler != nullptr) {
        _handler->handleGatewayConfiguration(mgc);
    }
}
...
void ts::mcast::NIPDemux::processSIF(const ServiceInformationFile& sif)
{
    ...
    // Finally forward to the NIP handler.
    if (_handler != nullptr) {
        _handler->handleServiceInformationFile(sif);
    }
}
...
void ts::mcast::NIPDemux::processSLEP(const ServiceListEntryPoints& slep)
{
    ...
    // Finally forward to the NIP handler.
    if (_handler != nullptr) {
        _handler->handleServiceListEntryPoints(slep);
    }
}
...
void ts::mcast::NIPDemux::processServiceList(const ServiceList& slist)
{
    ...
    // Finally forward to the NIP handler.
    if (_handler != nullptr) {
        _handler->handleServiceList(slist);
        for (const auto& it : new_services) {
            _handler->handleNewService(_services[it]);
        }
    }
}
