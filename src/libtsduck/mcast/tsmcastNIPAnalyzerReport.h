...
namespace ts::mcast {
    ...
    //!
    class TSDUCKDLL NIPAnalyzerReport : private FluteHandlerInterface
    {
    ...
    private:
        ...
        void processSIF(const ServiceInformationFile&);
        void processSLEP(const ServiceListEntryPoints&);
        void processServiceList(const ServiceList&);
        void processGatewayConfiguration(const GatewayConfiguration&);
    };
}
...
template <class Rep, class Period>
void ts::mcast::NIPAnalyzerReport::feedPacket(const cn::duration<Rep,Period>& timestamp, const IPPacket& pkt)
{
    if (pkt.isUDP()) {
...
// Feed the analyzer with a UDP packet (IPPacket version).
template <class Rep, class Period>
void ts::mcast::NIPAnalyzerReport::feedPacket(const cn::duration<Rep,Period>& timestamp, const IPSocketAddress& source, const IPSocketAddress& destination, const uint8_t* udp, size_t udp_size)
{
