...
bool ts::DektecOutputPlugin::start()
{
    ...
    if (present(u"instant-detach") && present(u"wait-detach")) {
        error(u"options --instant-detach and --wait-detach are mutually exclusive.");
        return false;
    }
    ...
    // Open the device
    debug(u"attaching to device %s serial 0x%X", _guts->device.model, _guts->device.desc.m_Serial);
    Dtapi::DTAPI_RESULT status = _guts->dtdev.AttachToSerial(_guts->device.desc.m_Serial);
    if (status != DTAPI_OK) {
        error(u"error attaching output Dektec device %d (%s): %s", _guts->dev_index, _guts->device.model, DektecStrError(status));
        return false;
    }
    ...
    if (_guts->power_mode >= 0) {
        debug(u"SetIoConfig(port: %d, group: %d, value: %d)", port, DTAPI_IOCONFIG_PWRMODE, _guts->power_mode);
    ...
    // Open the channel
    debug(u"attaching to port %d", port);
    status = _guts->chan.AttachToPort(&_guts->dtdev, port);
    if (status != DTAPI_OK) {
        error(u"error attaching output channel %d of Dektec device %d (%s): %s", _guts->chan_index, _guts->dev_index, _guts->device.model, DektecStrError(status));
        _guts->dtdev.Detach();
        return false;
    }
    ...
    debug(u"resetting channel, mode: %d", DTAPI_FULL_RESET);
    ...
    if (_guts->iostd_value >= 0) {
        debug(u"setting IO config of port %d, group: %d, value: %d, subvalue: %d", port, DTAPI_IOCONFIG_IOSTD, _guts->iostd_value, _guts->iostd_subvalue);
    ...
    debug(u"setting TxMode, tx: %d, stuff: %d", tx_mode, stuff_mode);
    ...
    if ((dt_flags & DTAPI_CAP_IP) != 0) {
        ...
        debug(u"setting IP parameters: DtIpPars2 = {");
    ...
    if (present(u"level")) {
        ...
        debug(u"set output level to %d", level);
        ...
        if (status != DTAPI_OK) {
            ...
            error(u"set modulator output level: %s", DektecStrError(status));
        }
    }
    ...
    if (status != DTAPI_OK || _guts->max_fifo_size == 0) {
        ...
        verbose(u"max fifo size not supported, using %'d bytes", _guts->max_fifo_size);
    }
    ...
    if (present(u"fifo-size")) {
        ...
        if (size > 0) {
            verbose(u"setting output fifo size to %'d bytes", size);
    ...
    verbose(u"output fifo size: %'d bytes, max: %'d bytes, typical: %'d bytes", _guts->fifo_size, _guts->max_fifo_size, typ_fifo_size);
    ...
    if (present(u"preload-fifo-delay")) {
        ...
        if (_guts->preload_fifo_delay && !setPreloadFIFOSizeBasedOnDelay()) {
            ...
            verbose(u"For --preload-fifo-delay, no bitrate currently set, so will use --preload-fifo-percentage settings until a bitrate has been set.");
        }
    }
    ...
    if (!_guts->preload_fifo_size) {
        ...
        if (_guts->maintain_preload && _guts->drop_to_maintain) {
            ...
            if ((_guts->preload_fifo_size + _guts->maintain_threshold) > _guts->fifo_size) {
                ...
                verbose(u"For --preload-fifo-percentage (%d), reducing calculated preload size from %'d bytes to %'d bytes to account for %'d byte threshold "
    ...
    if (_guts->cur_bitrate == 0) {
        warning(u"no input bitrate is available, use --bitrate in case of output error");
    }
    ...
    debug(u"setting TxControl to %d", tx_control);
    status = _guts->chan.SetTxControl(tx_control);
    if (status != DTAPI_OK) {
        return startError(u"output device start send error", status);
    }

    verbose(u"initial output bitrate: %'d b/s", _guts->cur_bitrate);
    if (_guts->starting) {
        verbose(u"Will preload FIFO before starting transmission. Preload FIFO size: %'d bytes.", _guts->preload_fifo_size);
    }
    else {
        verbose(u"Will start transmission immediately.");
    }
...
{
    if (status != DTAPI_OK) {
        error(u"%s: %s", message, DektecStrError(status));
    }
...
{
    int bitrate = -1;
    debug(u"DtapiModPars2TsRate(..., %d, %d, %d, %d, %d)", dt_modulation, param0, param1, param2, symbol_rate);
    ...
    if (status != DTAPI_OK) {
        ...
        verbose(u"setting output TS bitrate to %'d b/s", bitrate);
...
void ts::DektecOutputPlugin::displaySymbolRate(const BitRate& ts_bitrate, int dt_modulation, int param0, int param1, int param2)
{
    if (ts_bitrate > 0) {
        ...
        debug(u"DtapiModPars2SymRate(..., %d, %d, %d, %d, %d/%d)", dt_modulation, param0, param1, param2, frac_bitrate.m_Num, frac_bitrate.m_Den);
        Dtapi::DTAPI_RESULT status = Dtapi::DtapiModPars2SymRate(symrate, dt_modulation, param0, param1, param2, frac_bitrate);
        if (status != DTAPI_OK) {
            debug(u"DtapiModPars2SymRate using DtFractionInt failed, using int: %'d", DektecStrError(status), ts_bitrate.toInt());
            debug(u"DtapiModPars2SymRate(..., %d, %d, %d, %d, %d)", dt_modulation, param0, param1, param2, ts_bitrate.toInt());
            status = Dtapi::DtapiModPars2SymRate(symrate, dt_modulation, param0, param1, param2, int(ts_bitrate.toInt()));
        }
        if (status != DTAPI_OK) {
            verbose(u"error computing symbol rate: ", DektecStrError(status));
        }
        else {
            verbose(u"output symbol rate: %'d symbols/second", symrate);
        }
    }
}
...
bool ts::DektecOutputPlugin::setBitrate(const BitRate& bitrate)
{
    ...
    debug(u"SetTsRateBps(%d/%d), ie. %f", frac_bitrate.m_Num, frac_bitrate.m_Den, bitrate);
    Dtapi::DTAPI_RESULT status = _guts->chan.SetTsRateBps(frac_bitrate);
    if (status == DTAPI_E_NOT_SUPPORTED) {
        debug(u"setting TsRateBps using DtFractionInt unsupported, using int, SetTsRateBps(%d),", bitrate.toInt());
    ...
    if (status == DTAPI_OK) {
        ...
        error(u"output device set bitrate error: %s", DektecStrError(status));
        return false;
    }
}
...
bool ts::DektecOutputPlugin::setModulation(int& modulation_type)
{
    ...
    if (input != nullptr) {
        debug(u"found input modulator parameters: %s", input->toPluginOptions());
    ...
    switch (modulation_type) {
        ...
        case DTAPI_MOD_DVBS_BPSK: {
            ...
            verbose(u"using DVB-S FEC " + DektecFEC().name(fec));
            ...
            debug(u"SetModControl(%d, %d, %d, %d)", modulation_type, fec, 0, 0);
        ...
        case DTAPI_MOD_DVBS2_32APSK: {
            ...
            debug(u"SetModControl(%d, %d, %d, %d)", modulation_type, fec, param1, gold_code);
        ...
        case DTAPI_MOD_QAM256: {
            ...
            debug(u"SetModControl(%d, %d, %d, %d)", modulation_type, j83, qam_b, 0);
        ...
        case DTAPI_MOD_DVBT: {
            ...
            verbose(u"using DVB-T FEC %s, bandwidth %s, constellation %s, guard %s, transmission %s",
                         DektecFEC().name(fec),
            ...
            debug(u"SetModControl(%d, %d, %d, %d)", modulation_type, fec, param1, cell_id);
        ...
        case DTAPI_MOD_DVBT2: {
            ...
            // Report actual parameters in debug mode
            debug(u"DVB-T2: DtDvbT2Pars = {");
            DektecDevice::ReportDvbT2Pars(pars, *tsp, Severity::Debug, u"  ");
            debug(u"}");
            if (!(use_input_modulation && input == nullptr && _guts->cur_bitrate > 0)) {
                debug(u"DVB-T2: DtDvbT2ParamInfo = {");
            ...
            verbose(u"setting output TS bitrate to %'d b/s", _guts->cur_bitrate);
            // Set modulation parameters
        ...
        case DTAPI_MOD_ATSC: {
            ...
            verbose(u"using ATSC %s", DektecVSB().name(constel));
            debug(u"SetModControl(%d, %d, %d, %d)", modulation_type, constel, taps, 0);
        ...
        case DTAPI_MOD_DMBTH: {
            ...
            debug(u"SetModControl(%d, %d, %d, %d)", modulation_type, bw | constel | fec | header | interleaver | pilots | frame_num, 0, 0);
    ...
    verbose(u"setting output carrier frequency to %'d Hz", frequency);
...
{
    if (_guts->is_started) {
        verbose(u"terminating %s output", _guts->device.model);

        // Mute output signal for modulators which support this
        if (_guts->mute_on_stop) {
            debug(u"SetRfMode(%d)", DTAPI_UPCONV_MUTE);
            Dtapi::DTAPI_RESULT status = _guts->chan.SetRfMode(DTAPI_UPCONV_MUTE);
            if (status != DTAPI_OK) {
                error(u"error muting modulator output: " + DektecStrError(status));
            }
        }

        // Detach the channel and the device
        debug(u"detach channel, mode: %d", _guts->detach_mode);
        _guts->chan.Detach(_guts->detach_mode);
        debug(u"detach device");
        _guts->dtdev.Detach();

        _guts->is_started = false;
        verbose(u"%s output terminated", _guts->device.model);
    }
    return true;
}
...
ts::BitRate ts::DektecOutputPlugin::getBitrate()
{
    ...
    if (_guts->is_started) {
        ...
        if (status != DTAPI_OK) {
            error(u"error getting Dektec device output bitrate: %s", DektecStrError(status));
            bitrate = 0;
        }
    }
    return bitrate;
...
bool ts::DektecOutputPlugin::send(const TSPacket* buffer, const TSPacketMetadata* pkt_data, size_t packet_count)
{
        ...
        setBitrate(new_bitrate))
    {
        ...
        verbose(u"new output bitrate: %'d b/s", _guts->cur_bitrate);

        if (setPreloadFIFOSizeBasedOnDelay()) {
            verbose(u"Due to new bitrate and specified delay of %d ms, preload FIFO size adjusted: %'d bytes.", _guts->preload_fifo_delay, _guts->preload_fifo_size);
            if (_guts->maintain_threshold) {
                verbose(u"Further, maintain preload threshold for dropping packets set to %'d bytes based on bitrate.", _guts->maintain_threshold);
            }
        }
    }
    ...
    while (remain > 0) {
        ...
        if (_guts->starting) {
            ...
            if (status != DTAPI_OK) {
                error(u"error getting output fifo load: %s", DektecStrError(status));
                return false;
            }
            ...
            if (fifo_load < _guts->preload_fifo_size - int(PKT_SIZE)) {
                ...
                verbose(u"%s output FIFO load is %'d bytes, starting transmission", _guts->device.model, fifo_load);
                status = _guts->chan.SetTxControl(DTAPI_TXCTRL_SEND);
                if (status != DTAPI_OK) {
                    error(u"output device start send error: %s", DektecStrError(status));
                    return false;
                }
                // Now fully started
        ...
        while (!_guts->starting) {
            ...
            if (_guts->preload_fifo && _guts->maintain_preload) {
                if (fifo_load == 0) {
                    ...
                    _guts->starting = true;
                    verbose(u"Pausing transmission temporarily in order to maintain preload");
                }
                    ...
                    if ((_guts->drop_to_preload && ((fifo_load + cursize) > _guts->preload_fifo_size)) ||
                        ...
                        if (!_guts->drop_to_preload) {
                            ...
                            verbose(u"Would have exceeded preload FIFO size (%'d bytes) + threshold (%'d bytes), dropping packets to get back to preload FIFO size",
                        ...
                        if (excess >= cursize) {
                            verbose(u"Dropping all remaining packets (%'d bytes) to maintain preload FIFO size (%'d, %'d, %'d).",
                        ...
                        verbose(u"Dropping %'d bytes worth of packets to maintain preload FIFO size (%'d, %'d, %'d, %'d).",
                        ...
                        verbose(u"Got FIFO load (%'d bytes) + new packet data (%'d bytes) back down to preload FIFO size (%'d bytes) by dropping packets.",
        ...
        if (status != DTAPI_OK) {
            error(u"transmission error on Dektec device: %s", DektecStrError(status));
            return false;
        }
            ...
            if (status == DTAPI_OK) {
                if (latched) {
                    if ((latched & DTAPI_TX_CPU_UFL) != 0) {
                        verbose(u"Got CPU underflow.");
                    }
                    if ((latched & DTAPI_TX_DMA_UFL) != 0) {
                        verbose(u"Got DMA underflow.");
                    }
                    if ((latched & DTAPI_TX_FIFO_UFL) != 0) {
                        verbose(u"Got FIFO underflow.");
                    }
...
bool ts::DektecOutputPlugin::setPreloadFIFOSizeBasedOnDelay()
{
    if (_guts->preload_fifo_delay && _guts->cur_bitrate != 0) {
        ...
        if ((prelimPreloadFifoSize + uint64_t(_guts->maintain_threshold)) > uint64_t(_guts->fifo_size)) {
            ...
            if (_guts->maintain_threshold) {
                verbose(u"For --preload-fifo-delay, delay (%d ms) too large (%'d bytes), based on bitrate (%'d b/s) and FIFO size (%'d bytes). "
