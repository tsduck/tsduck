...
namespace ts {
    ...
    //!
    TSCOREDLL uint32_t DecodeBCD(const uint8_t* bcd, size_t bcd_count, bool left_justified = true);
    ...
    //! This parameter is ignored when @a bcd_count is even.
    //!
    TSCOREDLL void EncodeBCD(uint8_t* bcd, size_t bcd_count, uint32_t value, bool left_justified = true, uint8_t pad_nibble = 0);
    ...
    //! BCD digit starts in the second half of the first byte. Ignored when @a bcd_count is even.
    //!
    TSCOREDLL void BCDToString(std::string &str, const uint8_t* bcd, size_t bcd_count, int decimal, bool left_justified = true);
}
