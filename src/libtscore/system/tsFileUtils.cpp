...
ts::UString ts::VernacularFilePath(const UString& path)
{
    ...
    if (vern.length() >= 6 && vern.starts_with(u"/mnt/") && IsAlpha(vern[5]) && (vern.length() == 6 || vern[6] == u'/')) {
        vern.erase(0, 4);
    }

    // With Cygwin, the syntax "/cygdrive/C/" means "C:\"
    if (vern.starts_with(u"/cygdrive/")) {
        vern.erase(0, 9);
    }
...
{
#if defined(TS_WINDOWS)
    return path.starts_with(u"\\\\") || (path.length() >= 3 && IsAlpha(path[0]) && path[1] == u':' && path[2] == u'\\');
#else
...
ts::UString ts::BaseName(const UString& path, const UString& suffix)
{
    ...
    const bool suffixFound = !suffix.empty() && base.ends_with(suffix, FILE_SYSTEM_CASE_SENSITVITY);
