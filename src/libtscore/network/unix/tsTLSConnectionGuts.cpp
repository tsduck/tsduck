...
int ts::TLSConnection::SystemGuts::processStatus(const UChar* func, int status)
{
    const int err = SSL_get_error(ssl, status);
    conn->report().debug(u"OpenSSL: %s returned %d, error: %d", func, status, err);
    return err;
}
...
bool ts::TLSConnection::SystemGuts::abort(const UString& message)
{
    if (!message.empty()) {
        conn->report().error(message);
    }
...
bool ts::TLSConnection::connect(const IPSocketAddress& addr)
{
    ...
    if ((_guts->ssl = ::SSL_new(_guts->ssl_ctx)) == nullptr) {
        return _guts->abort(u"error creating TLS client connection context");
    }
    ...
    if (::SSL_set_fd(_guts->ssl, getSocket()) <= 0) {
        return _guts->abort(u"error setting file descriptor in TLS client context");
    }

    // Perform TLS handshake with the server.
    if (::SSL_connect(_guts->ssl) <= 0) {
        return _guts->abort(u"error in TLS handshake with server");
    }
...
bool ts::TLSConnection::setServerContext(const void* ssl)
{
    _guts->terminate();
...
bool ts::TLSConnection::disconnect(bool silent)
{
    ...
    while (_guts->shutdown_count > 0) {
        ...
        _guts->processStatus(u"SSL_shutdown", ret);
        if (ret > 0) {
            // Shutdown complete.
            _guts->shutdown_count = 0;
        }
...
bool ts::TLSConnection::send(const void* data, size_t size)
{
    ...
    if (ret <= 0) {
        _guts->processStatus(u"SSL_write", ret);
...
bool ts::TLSConnection::receive(void* buffer, size_t max_size, size_t& ret_size, const AbortInterface* abort)
{
    ...
    else if (_guts->processStatus(u"SSL_read_ex", ret) == SSL_ERROR_ZERO_RETURN) {
