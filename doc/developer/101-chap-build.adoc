...
By default, on all operating systems, the SRT library is the Haivision libsrt
...
By default, TSDuck is built with the original libsrt, as installed on the build
system. It is possible to build TSDuck with the Robotweax SRT library, even if
the standard libsrt is installed on the build system. Because the Robotweax SRT
library is not, or not yet, available as a standard package in common operating
systems, you have to build it locally first.
