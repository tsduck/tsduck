...
The first two documents are written in Asciidoc format.
Their HTML and PDF versions are built using Asciidoctor.
...
These two guides are installed with TSDuck, HTML and PDF version, on {unix} and Windows.
...
This reference is built using Doxygen.
...
the target `docs` is automatically as part of `make install`.
