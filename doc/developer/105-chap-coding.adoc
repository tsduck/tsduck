...
[.compact-list]
* LGPL libraries can be accessed through dynamic linking.
* LGPL libraries cannot be accessed through static linking or dynamic loading (`dlopen()`).
...
(In practice, `libedit` has been developed only because of the unfriendly license of `libreadline`).
...
Please consider that the main author of TSDuck is named "Lelégard", with an accent on the second "e",
