...
The workflow `nightly-build.yml` is automatically run every night (UTC).
Note that the load of the GitHub CI/CD pipeline is increasing every year
and the scheduled time is sometimes delayed by several hours.
Therefore, there is no guarantee on the publication time.

* On Linux Ubuntu and Windows (64-bit Intel only), the TSDuck binary packages are built
...
At the end of the nightly build workflow, when all build jobs are completed,
