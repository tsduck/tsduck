...
* Rogue signalization: Some organizations ignore this rule and define their own private descriptors,
  in the range 0x80-0xFE, without officially allocated PDS value. They use their own arbitrary PDS value,
  hoping that no one else will use the same PDS value.
...
In that descriptor, you will have to create four or five files (the last is optional).
