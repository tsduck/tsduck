...
==== Building applications on UNIX systems (Linux, macOS, BSD)
...
(OpenSSL on UNIX systems, BCrypt on Windows).
...
Therefore, TSDuck exclusively use `ts::Thread` and `ts::ThreadAttributes` instead.
...
This class is built on top of native system libraries (_libcurl_ on UNIX systems, _WinInet_ on Windows).
...
* On UNIX systems (Linux, macOS, BSD), this new source file will be automatically recognized
...
This script can be run on UNIX systems or Windows systems.
