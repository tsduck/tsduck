...
===== Creating the FreeBSD port

On FreeBSD systems, TSDuck is managed as a "port". This is how TSDuck can be installed
using the command `pkg install tsduck`.

Each time a new release is created, the maintainer shall request an update of the
TSDuck port. See the detailed description in the
https://github.com/tsduck/tsduck/tree/master/pkg/freebsd[pkg/freebsd] directory.
