...
streams which omit the initial DVB sequence for character table selection when using a
non-default character set.

[.optdoc]
By default, standard DVB encoding is used and ISO-6937 encoding is assumed in the absence of
initial sequence for character set selection.
...
The available table names for option `--default-charset` are listed below.
