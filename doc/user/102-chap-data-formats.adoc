...
See the documentation of the plugins xref:dvb-plugin[`dvb`], xref:dektec-input[`dektec`], xref:ip-input[`ip`],
xref:http-input[`http`], xref:hls-input[`hls`], xref:srt-input[`srt`], xref:rist-input[`rist`]
in xref:chap-plugins[xrefstyle=short] for more details on the reception of live transport streams.
...
Additionally, output plugins are provided for HiDes and AstroMeta modulator devices.
...
The trailer is not part of its packet.
...
Read the reference documentation section of each plugin in xref:chap-plugins[xrefstyle=short] for more details.
...
* *64-bit fixed-point value with 1 decimal digit*:
  ...
  The accuracy is better than with integers but with one decimal only.
...
On Linux and macOS, the `make` command accepts direct parameters.
Each of the following commands rebuilds TSDuck with a specific representation for bitrates.
Use the appropriate one for your needs.
...
When using a fixed-point representation for bitrates, it is possible to specify the number of decimal digits.
As an example, the last command rebuilds with fixed-point and three decimal digits instead of one.
...
There are three main file formats for PSI/SI: binary section files, XML and JSON text files.

These formats are documented in the next sections.
...
The utility `tstabcomp`, the table compiler, can translate between the various formats.

Some key differences between the formats are summarized in the following table.

.Differences between binary, XML, and JSON formats

[cols="<1,<1",stripes=none]
|===
|Binary section format |XML and JSON format

|Collections of individual sections in any order, not necessarily complete tables.
|Complete tables only.

|Exact representation, byte by byte, of sections which were extracted from a transport stream.
|High-level representation of tables.

|Not easily modifiable.
|Text which can be manually edited using any text editor or XML/JSON edition tool.

|===

Note that the JSON format is formally equivalent to XML.
...
An XML file containing PSI/SI tables for TSDuck shall be encoded in UTF-8 format to allow
...
All XML files use `<tsduck>` as root node.
...
[.optdoc]
This is a handy shortcut when working on Chinese transport streams.
...
[.optdoc]
Several options `--default-registration` can be specified.
...
[.optdoc]
Assume that the transport stream is a DTMB one.
...
(this results in an invalid PAT but this is for the demonstration only).
