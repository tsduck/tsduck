...
The next special attributes define _symbols_ and _conditions_.
Symbols are just names which can be defined in a global repository.
...
Currently, an `EXPRESSION` is just a symbol, possibly preceded with a negation operator `!`.
The expression `NAME` is true if the symbol `NAME` is defined at the point of evaluation.
Similarly, the expression `!NAME` is true if the symbol `NAME` is _not_ defined.

In future versions, more complex forms of expression may be added.
