...
|Availability of a powerful shell environment. TSDuck is a lightweight toolkit with elementary
 ...
 UNIX utilities (`grep`, `sed`, `awk`, etc.) See some complex examples in xref:chap-examples[xrefstyle=short].
...
|Currently no support for hardware tuners and Dektec devices.
 Recommended only when dealing with transport stream files, IP networking or AstroMeta-based modulators.
...
On FreeBSD, TSDuck is included in the Ports packages.
...
All tools are accessible from `/usr/local/bin`, as any other command from Ports.
The development environment is always installed with TSDuck.

==== Other BSD systems

There is no binary package for TSDuck on other BSD systems.
It must be compiled and installed using the `gmake` command.
See the chapter xref:tsduck-dev.html#chap-build["building and installing TSDuck"] section in <<TSDuck-Dev>>.

After installation, all tools are in `/usr/local/bin` for OpenBSD and DragonFly BSD.
