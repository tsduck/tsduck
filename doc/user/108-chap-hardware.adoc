...
The device path remains constant for a given physical tuner.
...
AstroMeta (formerly known as VATek) is a company from Taiwan which designs modulator chips.
...
There is no "device name" for AstroMeta-based modulators, only device indexes, from 0 to N-1
(with N being the number of AstroMeta-based devices in the system).

==== Tested devices

The tested devices were based on the A3 chip from AstroMeta:

[.compact-list]
* A3 developer board from AstroMeta.
* Suntechtv U3 USB modulator (see <<Suntech>>).

Note that the software (the `tsvatek` command and the `vatek` output plugin)
is developed and maintained by AstroMeta and controls all devices using AstroMeta chips.
