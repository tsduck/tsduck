...
|HOME
|On UNIX systems (Linux, macOS, BSD), used to locate user-specific TSDuck configuration files
 (see xref:conf-file-loc[xrefstyle=short] and xref:chan-usage[xrefstyle=short]).
...
|LD_LIBRARY_PATH
|On UNIX systems (Linux, macOS, BSD), used to locate shareable libraries such as `tsp` plugins or TSDuck extensions.
...
|PATH
|On UNIX systems (Linux, macOS, BSD), the system-defined search path of executables is also
...
|TS_CURL_RETRY
|On UNIX systems (Linux, macOS, BSD), define a retry policy for Web requests (HTTP, HLS, etc.)
 See xref:web-retry[xrefstyle=short] below.

|TS_DEBUG_OPENSSL
|On UNIX systems (Linux, macOS, BSD), display OpenSSL error messages on standard error.
...
On UNIX systems (Linux, macOS), the directory names are separated with colons ‘:’.
On Windows systems, they are separated with semi-colons ‘;’.

[#web-retry]
==== Web requests retry policy

On UNIX systems (Linux, macOS, BSD), TSDuck uses `libcurl` to perform Web requests.
