...
include::{docdir}/opt/group-duck-context.adoc[tags=!*;notitle;cas;std;hf;charset;timeref;pds]
...
** If the environment `TSPLUGINS_PATH` is defined, a list of directories is parsed.
   Directories are separated by a semicolon `;` on Windows and a colon `:` on UNIX systems.
