...
[.optdoc]
If the specified file is an existing directory, the output files are created in that directory
with the same name as input, with extension `.xml`, `.json`, or `.yml`, as appropriate.
If several input files are specified without `--merge`, the specified output must be a directory.

[.optdoc]
An output file is produced only if at least one of `--patch`, `--reformat`, `--uncomment`,
`--merge`, `--json`, or `--yaml` is specified.
