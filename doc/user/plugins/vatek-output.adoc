...
[.cmd-header]
Send the transport stream to an AstroMeta-based modulator device

This plugin sends the output transport stream to a modulator device
which is based on chips from https://www.astrometa.com.tw/[AstroMeta].
...
The name of the plugin remained `vatek` for compatibility only.
...
[.optdoc]
Device index, from 0 to N-1 (with N being the number of AstroMeta-based devices in the system).

[.optdoc]
Use the command `tsvatek --all` to have a complete list of devices in the system.

[.optdoc]
By default, use the first AstroMeta-based device (index 0).
