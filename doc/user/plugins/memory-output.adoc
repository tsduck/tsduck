...
It is useful only to C++, Java or Python developers who run an instance of `TSProcessor` pipeline inside their applications
...
To develop some custom form of transport stream output, C++ developers should rather develop their own output plugin.
...
[.compact-list]
* In C++, the event data is an instance of `PluginEventData` pointing to the output TS packets.
