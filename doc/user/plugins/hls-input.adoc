...
[.optdoc]
HLS encryption with method AES-128 is supported.
The decryption key files which are specified in the playlist must be accessible
without restriction other than cookies which are acquired with the playlist.
