...
By default, a log message is issued for each newly identified PID.
...
As another more radical example, using `-P filter --negate --label NN` deletes all TS packets until the PID is identified.
