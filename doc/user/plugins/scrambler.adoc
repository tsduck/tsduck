...
The control words are generated using
the default pseudo-random number generator of the operating system with additional security improvements.
...
When specific PID's are provided, fixed control words must be specified as well.
...
By default, scramble the specified service.
