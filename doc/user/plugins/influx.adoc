...
include::{docdir}/opt/group-clock.adoc[tags=!*]
...
A timestamp is always provided with each data entry.
...
The following table lists the possible types of the field `_measurement` and the semantics of the corresponding field `value`.
The `value` type is `float` (in practice, an integer value is provided).
...
|===
|_measurement |value |Tags |Secondary tags
...
Consider the following default metrics, sending the global bitrate of the TS.
...
* Field `value`: the bitrate in bits/second during the measurement period.
...
* Field `value`: the raw value of the last timestamp of that type in the measurement period.
* Tag `scope`: an identifier defining the scope of the bitrate value.
