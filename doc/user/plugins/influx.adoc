...
[.optdoc]
Send bitrate or PCR/PTS/DTS monitoring data for all PID's.
...
[.optdoc]
Send bitrate or PCR/PTS/DTS monitoring data for the specified PID's.
...
[.optdoc]
Send bitrate or PCR/PTS/DTS monitoring data for services.
...
In each bitrate entry, the field `_measurement` is `bitrate` and the tag `scope` indicates the type of entry.
...
In each entry, the field `_measurement` is one of `pcr`, `pts`, `dts`, depending on the type of timestamp.
The tag `scope` indicates the type of entry.
Depending on the value value of `scope`, a second tag is provided.

The field `_value` contains the raw value of the last timestamp of that type in the measurement period.

The following table lists the possible `scope` values and the corresponding second tag.

// PDF backend does not correctly process "autowidth" tables.
.InfluxDB timestamp metrics
ifndef::backend-pdf[]
[cols="<1m,<1m,<1,<1",stripes=none,options="autowidth"]
endif::[]
ifdef::backend-pdf[]
...
|===
|scope |Tag 2 |Tag 2 value |Condition

|pid
|pid
|PID in decimal.
|With `--pid` or `--all-pids`

|service
|service
|Service id in decimal, or service name.
|With `--service`
...
[source,text]
----
bitrate,scope=ts,tsid=4 value=23178552 1548161473059
pcr,scope=service,service=M6 value=1781951280555 1548161473059
pcr,scope=service,service=W9 value=1780346511814 1548161473059
pcr,scope=service,service=Arte value=1780520269610 1548161473059
pcr,scope=service,service=France\ 5 value=1782323879195 1548161473059
pcr,scope=service,service=6ter value=1788684323646 1548161473059
----
...
* Tag `scope`: an identifier defining the scope of the counter.
  Currently, the only defined value is `ts`, meaning that the counter includes all errors in the TS.
* Tag `tsid`: when `scope=ts` (currently the only defined value), this is the transport stream id as an integer.
