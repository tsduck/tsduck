...
By default, without better solution, the timestamp of a datagram is the system time when the `tsp` process
reads the datagram. This is better than nothing but not quite precise. We get the time when the `tsp` process
was scheduled _after_ receiving the packet, not the arrival time of the packet.

On Windows, this is the only solution (translation: do not use a Windows system to measure IAT).

On Linux and macOS, the kernel keeps a timestamp of the precise moment when the IP packet was
processed by the kernel and queued for later processing. This is much more precise than the
system time of the `tsp` process.

Additionally, on Linux, we can sometimes get an even better timestamp. Some NIC's can report a
hardware timestamp of the moment when the packet was received by the controller.
...
So, to get hardware timestamps, the feature must be supported by the hardware NIC _and_
its driver.
...
to be sure to get hardware timestamps.
