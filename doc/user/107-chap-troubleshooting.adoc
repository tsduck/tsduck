...
|TS_DEBUG_ARGS
|For developers using the TSCore library, display debug messages on standard error
 about command line arguments redefinition issues.

|TS_DEBUG_NEW_VERSION
|While searching for new versions of TSDuck (see `TSDUCK_NO_VERSION_CHECK`), display debug
 messages on standard error. See `TS_CERR_DEBUG_LEVEL` to enable debug messages no standard error.
...
|TS_REACTOR_TRACE
|When defined to any non-empty value, display debug messages about low-level details of the "reactor" classes
 (event dispatcher: epoll on Linux, kqueue on macOS and FreeBSD, IOCP on Windows).
...
|Default value for option `--ephemeral-rsa-bits`. See xref:opt-tls[xrefstyle=short].
...
[#win-ephemeral-keys]
*On Windows, lots of small files accumulate in `%APPDATA%\Microsoft\Crypto\Keys`*

[.optdoc]
This is likely due to TSDuck commands which open a TLS server with an ephemeral certificate and
which were killed before termination. On Windows, it is not possible to create a certificate with
a key which is in memory only. The key must be in a "named container" (i.e. a file). When the
application terminates, the key is properly deleted. However, if the application is killed or
interrupted before its normal termination, the key file remains.

[.optdoc]
*Solution*: From a PowerShell session, run command `tscleanuptempkeys.ps1`. This is a PowerShell
script which is installed with TSDuck. It deletes all remaining ephemeral keys which were created
as ephemeral RSA keys for TLS servers.

[.optdoc]
*Note*: This problem occurs on Windows only. On {unix}, the ephemeral key and certificate remain
in memory.
