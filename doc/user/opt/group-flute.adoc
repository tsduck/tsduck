...
ifdef::opt-delete-after[]
[.opt]
*--delete-after* _seconds_

[.optdoc]
ifdef::opt-flute[]
With `--extract-carousel`,
endif::[]
ifdef::opt-dvbnip[]
With `--save-dvb-gw`,
endif::[]
delete the extracted files the specified number of seconds after their creation.
This option is useful to prevent disk overflow when the file extraction runs continuously.
endif::[]
