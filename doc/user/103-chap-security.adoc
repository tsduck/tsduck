...
As TSDuck was used in production environments, with open networks, these features created opportunities for security issues.
...
* Use standard mechanisms which are accessible from the command line using `curl` or any standard Web or REST library.
...
3. Generate persistent self-signed certificates. This solution is not fully secure.
   On the client side, the identity of the server cannot be enforced and the connections are
   vulnerable to "Man-in-the-Middle attacks" (MitM). The option `--insecure` shall be specified
   in the client commands to avoid the validation of the server certificate.
4. Generate ephemeral self-signed certificates, on the fly, at each execution of a TSDuck
   command which opens a TLS server. The same security restrictions apply as in the previous case.
   It is easier because there is no need to use complex commands to generate persistent self-signed certificates.
   Each server automatically creates its own self-signed certificate on the fly.
...
To determine the type of certificate to use in a TLS server, the following algorithm is used:

* If `--ephemeral-rsa-bits` is specified, use an ephemeral certificate.
* If any of `--certificate-path`, `--key-path`, `--store` is specified, use the corresponding
  persistent certificate. On {unix}, `--certificate-path` and `--key-path` are used.
  On Windows systems, `--certificate-path` and `--store` are used. If only one is specified,
  the other one is read from its default value in environment variables.
* If environment variable `TSDUCK_TLS_EPHEMERAL_RSA_BITS` is defined, use an ephemeral certificate.
* The last alternative is using the persistent certificate as defined by environment variables
